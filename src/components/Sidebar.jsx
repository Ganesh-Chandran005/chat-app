...
export default function Sidebar({ rooms, activeRoomId, setActiveRoomId, setShowModal, isMobile }) {
  return (
    <div style={{ 
      ...
      <div style={{ padding: '16px 12px', borderBottom: '1px solid rgba(255, 255, 255, 0.05)', display: 'flex', alignItems: 'center', justifyContent: isMobile ? 'center' : 'space-between' }}>
        {!isMobile && (
        ...
        {rooms.map((room) => {
          ...
          return (
            <div 
              ...
              title={room.name}
            >
              <Hash style={{ width: '16px', height: '16px', color: isActive ? '#c084fc' : '#71717a', flexShrink: 0 }} />
              {!isMobile && (

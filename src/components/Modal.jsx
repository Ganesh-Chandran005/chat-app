  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0, 0, 0, 0.6)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div style={{ backgroundColor: '#0b082e', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '16px', padding: '24px', width: '90%', maxWidth: '400px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
        <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#ffffff', letterSpacing: '0.05em', textTransform: 'uppercase', margin: '0 0 4px 0' }}>Establish New Chat Vector</h3>
        <p style={{ fontSize: '12px', color: '#a1a1aa', margin: '0 0 20px 0' }}>Initialize an isolated message sequence parameter node.</p>
        <form onSubmit={handleCreateRoom} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <label style={{ display: 'block', fontSize: '10px', fontWeight: '700', color: '#c084fc', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Vector Designation Name</label>
            <input 
              type="text" 
              value={newRoomName}
          ...
          <div>
            <label style={{ display: 'block', fontSize: '10px', fontWeight: '700', color: '#c084fc', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Node Operational Description</label>
            <input 
              type="text" 
              value={newRoomDesc}
              onChange={(e) => setNewRoomDesc(e.target.value)}
              placeholder="e.g., Discussion array for code systems" 
              style={{ width: '100%', boxSizing: 'border-box', backgroundColor: 'rgba(0, 0, 0, 0.4)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '12px', padding: '10px 12px', fontSize: '14px', color: '#e4e4e7', outline: 'none' }}
            />
          </div>
